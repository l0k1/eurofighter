...
var terrain_map = {
	...
	init : func {
		print("Initialising MFD Moving Map");
	...
	update : func {
		...
		if (getprop("/instrumentation/efis/inputs/TERR")) {

			var pos_lon = getprop("/position/longitude-deg");
			var pos_lat = getprop("/position/latitude-deg");
			var heading = getprop("/instrumentation/MFD[0]/PA/orientation-deg");

			#setprop("/controls/mfd/terrain-map/range", getprop("/instrumentation/radar/range"));

			var range = getprop("/instrumentation/MFD[0]/PA/zoom-nm");
