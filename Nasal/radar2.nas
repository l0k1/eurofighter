...
var Radar = {
    ...
    init: func(){
        ...
        var loop_Update = func(){
			...
			#print("radarWorking: " ~ radarWorking);
    ...
    ## The code pout the aircraft properties if nothing has been passed in parameters
		...
		me.OurRoll = getprop("orientation/roll-deg");
   ...
   next_Target_Index: func(){
	...
	hud_nearest_tgt: func() {
		...
		if(size(me.tgts_list) != 0 and me.az_fld==me.focused_az_fld and me.tgts_list != nil)
        {
			...
			var nearest_u = me.tgts_list[ me.Target_Index];
				
				if ( nearest_u.get_display() ) {
						var u_target = nearest_u.type ~ "[" ~ nearest_u.index ~ "]";
						var our_pitch = me.OurPitch;
						var u_dev_rad = (90 - nearest_u.get_deviation(me.OurHdg, me.MyCoord)) * D2R;
						var u_elev_rad = (90 - nearest_u.get_total_elevation_from_Coord(me.OurPitch, me.MyCoord)) * D2R;
						...
						var combined_dev_deg = dev_deg - me.OurRoll;
						...
						var h = getprop("instrumentation/radar2/hud-vertical-offset-m");
						if (h != nil) { 
							var hud_voffset = h;
						} else {
							var hud_voffset = 0;
						...
						if ( hud_voffset == 0 ) {
								if ( combined_dev_length > hud_radius ) {
										...
										Clamp_Blinker.blink();
										MissIntSound.setValue(1);
										MissContSound.setValue(0);
								} else {
										Clamp_Blinker.cont();
										MissIntSound.setValue(0);
										MissContSound.setValue(1);
								}
						} else {
...
var MissIntSound      = props.globals.getNode("sim/model/eurofighter/controls/armament/missile/int-sound", 1);
var MissContSound     = props.globals.getNode("sim/model/eurofighter/controls/armament/missile/cont-sound", 1);
...
var Target = {
    new: func(c,theTree = nil){
		#print("CALLSINGING: " ~ c.getNode("callsign").getValue());
...
radar_mode_sel = func(mode) {
        ...
        if ( wcs_mode == "rws" ) {
                ...
                swp_diplay_width = 0.1125;
        } else {
                AzField.setValue(60);
                swp_diplay_width = 0.0563;
        }
}
